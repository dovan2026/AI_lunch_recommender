...
def search_restaurants(lat, lng, menu_names, backup_category, radius_m, max_results=10):
    ...
    # 최종 결과 거리 순으로 정렬 (가장 가까운 곳 우선)
    restaurants.sort(key=lambda x: int(x.get("distance", 9999)))
...
if recommend_btn and ready:
    ...
    with r_col2:
        ...
        st.markdown('<div style="color:rgba(255,255,255,0.35); font-size:0.75rem; margin-top:-0.5rem; margin-bottom:0.7rem;">⭐ 실제 별점 및 리뷰는 "카카오맵에서 보기" 링크를 통해 실시간으로 확인하실 수 있습니다.</div>', unsafe_allow_html=True)
