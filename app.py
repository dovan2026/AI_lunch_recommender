...
def search_restaurants(lat, lng, food_types, radius_m, max_results=12):
    headers = {"Authorization": f"KakaoAK {KAKAO_API_KEY}"}
    restaurants, seen = [], set()
    for ft in food_types:
        params = {"query": ft["query_keyword"], "x": str(lng), "y": str(lat),
...
if recommend_btn and ready:
    ...
    # ── AI 메뉴 추천 ──
    with r_col1:
        st.markdown('<div class="result-title">🤖 내장 메뉴 추천</div>', unsafe_allow_html=True)
        ai = None
    ...
    # ── 주변 맛집 ──
    with r_col2:
        st.markdown('<div class="result-title">🗺️ 주변 맛집(TOP 3)</div>', unsafe_allow_html=True)
        with st.spinner("카카오맵에서 검색하고 있어요..."):
            ...
            try:
                ...
                if lat is None:
                    ...
                    search_types = sel_food_objs if sel_food_objs else FOOD_TYPES[:3]
                    rests = search_restaurants(lat, lng, search_types, distance_val, max_results=3)
                    ...
                    if not rests:
                        ...
                        for i, r in enumerate(rests, 1):
                            ...
                            <div class="restaurant-card">
                                ...
                                <span style="color:#fbbf24; font-size:0.78rem; font-weight:700;">⭐ {rating:.1f} ({review_cnt})</span>
                                <div class="restaurant-info">
                                    <span>📏 {dist_str}</span><span class="info-dot"></span>
